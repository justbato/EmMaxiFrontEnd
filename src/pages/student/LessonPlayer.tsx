...
export function LessonPlayerPage() {
  ...
  return (
    <div className="flex h-screen bg-[var(--nav-bg)] transition-colors overflow-hidden">
      ...
      <div className={`${sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'} fixed lg:relative top-0 left-0 h-full w-[280px] bg-[#1a1a1a] border-r border-[rgba(255,255,255,.08)] flex flex-col z-50 transition-transform duration-300`}>
        ...
        <div className="p-4 border-b border-[rgba(255,255,255,.08)] flex-shrink-0">
          ...
          <div className="text-[13px] font-[600] text-[var(--text-primary)] leading-[1.3] mb-2">{course.title}</div>
              ...
              {section.lessons.map((lesson, i) => {
                ...
                return (
                  <div key={lesson.id} onClick={() => { setActiveLessonId(lesson.id); setSidebarOpen(false) }}
                    ...
                    <div className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] flex-shrink-0 mt-0.5 ${isDone ? 'bg-[#10B981] text-[var(--text-primary)]' : isActive ? 'bg-[#D4A017] text-[var(--text-primary)]' : 'border border-[var(--border)] text-[rgba(255,255,255,.3)]'}`}>
                      {isDone ? '✓' : i + 1}
                    </div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="bg-[#1a1a1a] border-b border-[rgba(255,255,255,.08)] px-4 py-3 flex items-center justify-between flex-shrink-0">
          <div className="flex items-center gap-3">
            <button onClick={() => setSidebarOpen(true)} className="lg:hidden text-[rgba(255,255,255,.6)] hover:text-[var(--text-primary)] transition-colors">
            ...
            <div>
              <div className="text-[13px] font-[600] text-[var(--text-primary)]">{activeLesson.title}</div>
        ...
        {/* Content */}
        <div className="flex-1 overflow-y-auto bg-[var(--bg-secondary)] transition-colors">
          {/* Video */}
          {(activeLesson.type === 'video' || activeLesson.type === 'both') && (
            <div className="bg-[var(--nav-bg)] transition-colors aspect-video max-h-[55vh] w-full">
              <iframe
          ...
          <div className="p-6 max-w-3xl mx-auto">
            <h1 className="text-[20px] font-[700] text-[var(--text-primary)] mb-4" style={{ fontFamily: 'Clash Display, sans-serif' }}>{activeLesson.title}</h1>

            {/* Text content */}
            {(activeLesson.type === 'text' || activeLesson.type === 'both') && activeLesson.content && (
              <div className="bg-[var(--surface)] transition-colors border border-[var(--border)] rounded-[14px] p-6 mb-6 text-[14px] text-[var(--text-secondary)] leading-[1.8]">
                <p>{activeLesson.content}</p>
                <div className="mt-4 p-4 bg-[#FDF6DC] dark:bg-[rgba(212,160,23,.12)] border border-[var(--border)] dark:border-[rgba(212,160,23,.2)] rounded-[8px]">
            ...
            {/* Q&A */}
            <div className="bg-[var(--surface)] transition-colors border border-[var(--border)] rounded-[14px] p-5">
              <h3 className="text-[14px] font-[700] mb-4">❓ Ask a question</h3>
              {qaSubmitted ? (
                <div className="bg-[#D1FAE5] dark:bg-[rgba(16,185,129,.15)] border border-[#86EFAC] rounded-[8px] p-4 text-[13px] text-[#065F46] font-[600] text-center">
                ...
                <>
                  <textarea
                    ...
                    className="w-full px-4 py-3 border-[1.5px] border-[var(--border-2)] rounded-[8px] text-[13px] outline-none focus:border-[#D4A017] transition-colors resize-y"
                  />
