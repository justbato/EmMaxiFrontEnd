  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row overflow-hidden bg-[var(--bg)]">
      <style>{`
        ...
        .demo-btn:hover { border-color: #D4A017 !important; background: var(--brand-glow) !important; }
      `}</style>
      ...
      <div className={`panel-left${mounted ? ' in' : ''} w-full md:flex-1 bg-[var(--bg-secondary)] text-[var(--text-primary)] p-8 md:p-12 flex flex-col justify-center transition-colors`}>
        <div className="max-w-sm mx-auto w-full">
          ...
          <p className="text-[var(--text-secondary)] text-[13px] leading-[1.8] mb-6 md:mb-8">
            Sign in to continue your learning journey or manage your courses.
          </p>

          {/* Demo accounts */}
          <div className="space-y-2">
            <div className="text-[11px] text-[var(--text-tertiary)] uppercase tracking-[.08em] font-[600] mb-3">
              Quick demo logins
            </div>
            {[
              ...
              <button key={d.email} onClick={() => demoLogin(d.email)}
                className="demo-btn w-full text-left p-3 rounded-[10px] bg-[var(--surface-hover)] border border-[var(--border)] transition-all">
                <div className="text-[12px] font-[600] text-[var(--text-primary)]">{d.label}</div>
                <div className="text-[10px] text-[var(--text-secondary)] mt-0.5">{d.desc}</div>
              </button>
            ))}
          </div>
        </div>
      ...
      {/* Right – form panel */}
      <div className={`panel-right${mounted ? ' in' : ''} w-full md:w-[440px] md:min-w-[440px] bg-[var(--surface)] border-t md:border-t-0 md:border-l border-[var(--border)] flex items-center justify-center p-8 md:p-10 md:shadow-[-4px_0_24px_rgba(0,0,0,.08)] transition-colors`}>
        <div className="w-full max-w-[360px]">
          <div className="text-[24px] font-[700] mb-1 text-[var(--text-primary)]" style={{ fontFamily: 'Clash Display, sans-serif' }}>Sign in</div>
          ...
          {/* Email */}
          <div className="mb-4">
            <label className="block text-[12px] text-[var(--text-secondary)] mb-1 font-[600]">Email</label>
            <input type="email" placeholder="you@example.com" value={email}
              onChange={e => { setEmail(e.target.value); setError('') }}
              className={`w-full px-[14px] py-[10px] bg-[var(--input-bg)] text-[var(--text-primary)] border-[1.5px] rounded-[8px] text-[13px] outline-none transition-all focus:border-[#D4A017] focus:shadow-[0_0_0_3px_var(--input-focus-ring)] ${emailErr ? 'border-[#EF4444]' : 'border-[var(--input-border)]'}`} />
          ...
          <div className="mb-2">
            <div className="flex items-center justify-between mb-1">
              <label className="text-[12px] text-[var(--text-secondary)] font-[600]">Password</label>
            ...
            <div className="relative">
              <input type={showPass ? 'text' : 'password'} placeholder="••••••••" value={password}
                ...
                className={`w-full px-[14px] py-[10px] pr-10 bg-[var(--input-bg)] text-[var(--text-primary)] border-[1.5px] rounded-[8px] text-[13px] outline-none transition-all focus:border-[#D4A017] focus:shadow-[0_0_0_3px_var(--input-focus-ring)] ${passErr ? 'border-[#EF4444]' : 'border-[var(--input-border)]'}`}
                onKeyDown={e => e.key === 'Enter' && handleLogin()} />
              <button onClick={() => setShowPass(!showPass)}
                className="absolute right-3 top-1/2 -translate-y-1/2 text-[var(--text-tertiary)] text-[12px]">
                {showPass ? '🙈' : '👁'}
              </button>
            </div>
