...
export function LandingPage() {
  ...
  useEffect(() => {
    ...
    document.querySelectorAll('.fade-up, .fade-left, .fade-right, .scale-in, .stat-pop').forEach(el => observerRef.current?.observe(el))
    ...
    <div className="min-h-screen bg-white text-text" style={{ fontFamily: 'Google Sans, Inter, sans-serif' }}>
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 24px', height: 64, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <div className="hidden md:flex" style={{ gap: 32 }}>
            {['Features', 'Courses', 'Reviews'].map(item => (
              <a key={item} href={`#${item.toLowerCase()}`} className="nav-link" style={{ fontSize: 14, fontWeight: 500, color: '#3c4043', textDecoration: 'none' }}
          ...
          <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
            <button onClick={() => navigate('/login')}
              style={{ fontSize: 14, fontWeight: 600, color: '#D4A017', background: 'none', border: 'none', cursor: 'pointer', padding: '8px 16px' }}>
              Sign in
            </button>
            <button onClick={() => navigate('/signup')}
              style={{ fontSize: 14, fontWeight: 600, color: '#fff', background: '#D4A017', border: 'none', borderRadius: 8, cursor: 'pointer', padding: '10px 20px' }}>
              Get started
            </button>
          </div>
        </div>
      ...
      <section style={{ background: '#f8f9fa', padding: '48px 24px' }}>
        <div style={{ maxWidth: 900, margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 32 }}>
          {[['24,000+', 'Active learners'], ['480+', 'Expert instructors'], ['1,800+', 'Courses available'], ['94%', 'Completion rate']].map(([num, lbl], i) => (
