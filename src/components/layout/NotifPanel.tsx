...
function NotifItem({ notif }: { notif: Notification }) {
  ...
  return (
    <div className={`rounded-[14px] p-3 border border-[#E8E8E8] bg-white relative transition-shadow animate-[pageSlideIn_.3s_both] border-l-[3px]`}
      style={{ borderLeftColor: borderColor }}>
      <div className="flex items-start gap-2.5">
        {notif.icon && <span className="text-[18px] flex-shrink-0">{notif.icon}</span>}
        <div className="flex-1">
          <div className="text-[12px] font-[700] text-[#111] mb-1">{notif.title}</div>
          <div className="text-[11px] text-[#333] leading-[1.6]">{notif.body}</div>
          <div className="text-[10px] text-[#6B6B6B] mt-1.5">📅 {notif.date}</div>
        </div>
...
export function NotifPanel() {
  ...
  return (
    <>
      ...
      <div
        id="notif-panel"
        className={`fixed top-0 right-0 w-[460px] max-w-[100vw] h-full bg-white z-[9500] shadow-[-8px_0_40px_rgba(0,0,0,.18)] flex flex-col transition-transform duration-[350ms] cubic-bezier(.22,1,.36,1) ${panelOpen ? 'translate-x-0' : 'translate-x-full'}`}
      >
        {/* Header */}
        <div className="px-5 pt-[18px] border-b border-[#E8E8E8] flex-shrink-0">
          <div className="flex items-center justify-between mb-0">
            <div>
              ...
              <div className="text-[11px] text-[#6B6B6B] mt-0.5">Your activity and messages</div>
            </div>
            <div className="flex items-center gap-2">
              ...
              <button onClick={closePanel} className="text-[20px] text-[#6B6B6B] leading-none px-1.5 py-0.5 hover:text-[#111] transition-colors">✕</button>
            </div>
          </div>
          {/* Tabs */}
            ...
            {(['all', 'account', 'reports', 'updates'] as const).map(tab => {
              ...
              return (
                <button key={tab} onClick={() => setActiveTab(tab)}
                  className={`flex-1 py-[9px] text-[12px] font-[600] border-none bg-none cursor-pointer border-b-[2.5px] transition-all ${activeTab === tab ? 'text-[#D4A017] border-b-[#D4A017]' : 'text-[#6B6B6B] border-b-transparent'}`}>
                  {labels[tab]}
        ...
        <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-2.5">
          {filtered.length === 0 ? (
            <div className="text-center py-12 text-[#6B6B6B]">
              ...
              <strong className="block text-[13px] text-[#333]">You're all caught up</strong>
