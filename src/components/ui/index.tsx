...
export function Btn({ variant = 'primary', size = 'md', className = '', children, ...props }: BtnProps) {
  ...
  const variants = {
    ...
    outline: 'bg-white text-[#111] border border-[#D0D0D0] hover:border-[#D4A017] hover:text-[#D4A017] hover:bg-[#FDF6DC] hover:-translate-y-px',
    ...
    ghost: 'bg-transparent text-[#6B6B6B] hover:bg-[#F5F5F5] hover:text-[#111]',
  }
  return (
    ...
    <button
      ...
      className={`filter-pill px-[14px] py-[6px] rounded-full border-[1.5px] text-[12px] font-[500] cursor-pointer transition-all ${
        active
          ...
          : 'border-[#D0D0D0] bg-white text-[#333]'
      } ${className}`}
    >
      {children}
    </button>
...
export function StarRating({ rating, count, size = 'sm' }: StarProps) {
  ...
  return (
    <span className={`flex items-center gap-1 ${sz}`}>
      ...
      <span className="font-[600]">{rating.toFixed(1)}</span>
      {count !== undefined && <span className="text-[#6B6B6B]">({count.toLocaleString()})</span>}
    </span>
  )
}
...
export function ProgressBar({ value, className = '', height = 6 }: ProgressProps) {
  return (
    <div className={`bg-[#EBEBEB] rounded-full overflow-hidden ${className}`} style={{ height }}>
...
export function Tag({ children }: { children: ReactNode }) {
  return <span className="inline-block px-[10px] py-[3px] rounded-full text-[11px] bg-[#FDF6DC] text-[#7A5A00] border border-[rgba(212,160,23,.3)] font-[500]">{children}</span>
}
...
export function Card({ children, className = '', onClick }: { children: ReactNode; className?: string; onClick?: () => void }) {
  return (
    <div
      onClick={onClick}
      className={`bg-white border border-[#E8E8E8] rounded-[14px] shadow-[0_1px_3px_rgba(0,0,0,.06),0_4px_16px_rgba(0,0,0,.05)] ${onClick ? 'cursor-pointer' : ''} ${className}`}
    >
      {children}
    </div>
  )
...
export function StatCard({ icon, iconBg, label, value, delta, deltaUp }: StatCardProps) {
  return (
    <div className="stat-card bg-white border border-[#E8E8E8] rounded-[14px] p-5 shadow-[0_1px_3px_rgba(0,0,0,.06),0_4px_16px_rgba(0,0,0,.05)] transition-all duration-[250ms]">
      <div className="w-10 h-10 rounded-[10px] flex items-center justify-center text-[18px] mb-3" style={{ background: iconBg }}>{icon}</div>
      <div className="text-[11px] text-[#6B6B6B] uppercase tracking-[.07em] mb-2 font-[600]">{label}</div>
      <div className="text-[24px] font-[700] font-[Clash_Display,sans-serif] text-[#111]">{value}</div>
...
export function Divider({ label }: { label?: string }) {
  if (!label) return <div className="h-px bg-[#E8E8E8] my-4" />
  return (
...
export function Input({ label, error, prefix, className = '', ...props }: InputProps) {
  return (
    <div className="mb-4">
      {label && <label className="block text-[12px] text-[#333] mb-1 font-[600] tracking-[.02em]">{label}</label>}
      <div className="relative">
        {prefix && <span className="absolute left-3 top-1/2 -translate-y-1/2 text-[#6B6B6B] text-[14px] font-[600] pointer-events-none z-10">{prefix}</span>}
        <input
          className={`w-full px-[14px] py-[10px] bg-white border-[1.5px] border-[#D0D0D0] rounded-[8px] text-[#111] font-[Satoshi,sans-serif] text-[13px] outline-none transition-all focus:border-[#D4A017] focus:shadow-[0_0_0_3px_rgba(212,160,23,.15)] ${error ? 'border-[#EF4444] focus:border-[#EF4444]' : ''} ${prefix ? 'pl-7' : ''} ${className}`}
          {...props}
        />
      </div>
...
export function Textarea({ label, error, className = '', ...props }: TextareaProps) {
  return (
    <div className="mb-4">
      {label && <label className="block text-[12px] text-[#333] mb-1 font-[600]">{label}</label>}
      <textarea
        className={`w-full px-[14px] py-[10px] bg-white border-[1.5px] border-[#D0D0D0] rounded-[8px] text-[#111] font-[Satoshi,sans-serif] text-[13px] outline-none transition-all resize-y min-h-[80px] focus:border-[#D4A017] focus:shadow-[0_0_0_3px_rgba(212,160,23,.15)] ${error ? 'border-[#EF4444]' : ''} ${className}`}
        {...props}
      />
...
export function Select({ label, options, className = '', ...props }: SelectProps) {
  return (
    <div className="mb-4">
      {label && <label className="block text-[12px] text-[#333] mb-1 font-[600]">{label}</label>}
      <select
        className={`w-full px-[14px] py-[10px] bg-white border-[1.5px] border-[#D0D0D0] rounded-[8px] text-[#111] font-[Satoshi,sans-serif] text-[13px] outline-none transition-all focus:border-[#D4A017] ${className}`}
        {...props}
      >
